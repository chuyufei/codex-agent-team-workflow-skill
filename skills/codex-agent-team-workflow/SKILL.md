...
1. Inspect the current project state first: project docs, `AGENTS.md`, git status, test commands, and any user-provided transcript or requirements.
2. Decide whether multi-agent coordination is worth the overhead. For a tiny one-file fix, keep the workflow simple and state that choice.
3. Create or update durable coordination artifacts:
...
4. If Codex thread-management tools are available, use them to create/list/read/rename threads and send messages. If not, use the same handoff protocol in Markdown and ask the user to copy messages between threads.
5. Run the loop:
...
6. Main/coordinator agent summarizes the path, evidence, tests, risks, and remaining work for the user.
...
- Read `references/video-principles.md` when preserving the transcript-derived ideas: long-lived threads, agent registry, message delivery, traceability, worklogs, asynchronous pipeline, and the three-agent minimum.
